...
/**
 * 测试WallApproach(靠近墙壁)指导行为
 * @author JhonKkk
 */
public class WallApproachTest extends FastApp implements ActionListener{
    ...
    WallApproachBehavior wallApproachBehavior;
    public static void main(String[] args) {
        new WallApproachTest(900, 600, "WallApproachTest(靠近指定墙壁)").start();
    }
    public WallApproachTest(int w, int h, String title) {
        super(w, h, title);
    }
...
//        bulletAppState.setDebugEnabled(true);
    ...
    public void simpleInitApp() {
        initBullet();
        Node scene = (Node) assetManager.loadModel("Scenes/WallApproachScene.j3o");
        Node wallNode = (Node) scene.getChild("wall");
        ...
        bulletAppState.getPhysicsSpace().add(wallNode.getControl(PhysicsControl.class));
        playAnim("Idle", .5f);
        //创建代理
        ...
        float offsetToMaintain = 0.1f;
        wallApproachBehavior = new WallApproachBehavior(npcAgent, wallNode, offsetToMaintain);
        wallApproachBehavior.setVelocity(new Vector3f(0, 0, 0));
        wallApproachBehavior.setEnabled(false);
        ...
        simpleMainBehavior.addBehavior(wallApproachBehavior);
        //为代理设置主指导行为
        ...
        executor.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                ...
                if(results.size() > 0){
                    ...
                    wallApproachBehavior.setEnabled(true);
                }
                else{
