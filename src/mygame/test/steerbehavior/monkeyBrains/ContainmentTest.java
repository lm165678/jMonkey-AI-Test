...
/**
 * 测试Containment(围堵)指导行为
 * @author JhonKkk
 */
public class ContainmentTest extends FastApp implements ActionListener{
    ...
    ContainmentBehavior containmentBehavior;
    public static void main(String[] args) {
        new ContainmentTest(900, 600, "ContainmentTest(围堵指定标记)").start();
    }
    public ContainmentTest(int w, int h, String title) {
        super(w, h, title);
    }
    ...
    protected void initMark() {
        ...
        mark.updateGeometricState();
        mark.updateModelBound();
    }
    ...
    public void simpleInitApp() {
        initBullet();
        Node scene = (Node) assetManager.loadModel("Scenes/ContainmentSteerScene.j3o");
...
//        playAgent = new Agent("playerAgent", new TargetAgentSpatial("targetAgentSpatial", npcAimingPoint, mark));
        ...
        containmentBehavior = new ContainmentBehavior(npcAgent, (Node) scene.getChild("ContainmentArea"));
        containmentBehavior.setVelocity(new Vector3f(1, 1, 1));
        containmentBehavior.setupStrengthControl(10, 0, 10);
        containmentBehavior.setEnabled(false);
        ...
        simpleMainBehavior.addBehavior(containmentBehavior);
        //为代理设置主指导行为
        ...
        executor.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                ...
                if(results.size() > 0){
                    ...
                    containmentBehavior.setEnabled(true);
                }
                else{
